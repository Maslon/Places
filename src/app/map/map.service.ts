...
export class MapService {
	...
	getCitySummary() {
		return wiki().page(this.cityName).then(page => page.summary()).then(result => result);
	}

	getWebsite() {
		return wiki().page(this.cityName).then(page => page.fullInfo()).then((result: Info) => {
			if (result.general.website && typeof result.general.website === 'string') {
				return result.general.website.split(' ')[0].slice(1);
			} else if (result.general.website) {
				return result.general.website[0];
			} else return '';
		});
	}
